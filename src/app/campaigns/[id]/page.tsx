...
export default function CampaignPage({ params }: { params: { id: string } }) {
  ...
  const loadCampaignData = useCallback(async () => {
    ...
    const { data: submissionData, error: submissionError } = await supabase
      .from('submissions')
      .select('id, post_url, view_count, created_at, profiles(username, avatar_url)')
    ...
    if (submissionError) {
      ...
      const formattedSubmissions = submissionData.map(s => ({...s, creator_name: s.profiles?.username, avatar_url: s.profiles?.avatar_url}));
